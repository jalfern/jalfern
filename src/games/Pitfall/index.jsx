...
const PitfallGame = () => {
    ...
    useEffect(() => {
        ...
        const update = () => {
            ...
            activeObjects.forEach(obj => {
                ...
                if (['fire', 'snake', 'scorpion'].includes(obj.type)) {
                    // Check overlap. These are static or semi-static on ground.
                    if (Math.abs(player.x - obj.x) < 15 && Math.abs(player.y - GROUND_Y) < 10) {
                        // Death
                        player.y = 800 // Trigger death logic below
                    }
                }
        ...
        const draw = () => {
            ...
            ctx.fillText(`PITCH: ${currentScreen}`, SCREEN_WIDTH - 40, 35)
